library angular_transformers.html_extractor;
...
class HtmlExpressionExtractor {
  final Set<String> expressions = new Set<String>();
  final Iterable<DirectiveInfo> directiveInfos;
  ...
  HtmlExpressionExtractor(this.directiveInfos) {
    for (DirectiveInfo directiveInfo in directiveInfos) {
      if (directiveInfo.template != null) {
        process(directiveInfo.template);
      }
    }
  }

  /**
   * Extracts all expressions from the provided HTML contents.
   */
  void process(String html) {
    var document = parse(html);
    _visitNodes([document], (Node node) {
  ...
  _visitNodes(List<Node> nodes, NodeVisitor visitor) {
    for (Node node in nodes) {
      visitor(node);
      if (node.nodes.length > 0) {
        _visitNodes(node.nodes, visitor);
      }
    }
  }
}
